...
var Exception = require('ecofyjs-exception').Exception;
	...
	var promise = promiseutils.createPromise(function(resolve, reject) {
		...
		resourceModel.save(function(error){
		    ...
		    if(error) {
		    	...
		    	logger.Logger.logException(this.logger_, exception, 'Error when saving to Mongo', logMeta);
		        reject(exception);
		    } else {
	...
	var promise = promiseutils.createPromise(function(resolve, reject) {
		this.Model_.find(criteria)
    		...
    		.exec(function(error, result) {
	    		...
	    		if(error) {
	    			var exception = Exception.wrapError(error);
	    			logger.Logger.logException(this.logger_, exception, 'Error querying Mongo', criteria);
			        reject(error);
			    } else {
			        resolve(result);
			    }
...
internals.DefaultProvider.prototype.update = function(criteria, resource, options)
{
	...
	var promise = promiseutils.createPromise(function(resolve, reject) {
		...
		this.Model_.findOneAndUpdate(criteria, resource, {upsert: false}, function(error, doc) {
		    ...
		    if (error) {
		    	var exception = Exception.wrapError(error);
	    		logger.Logger.logException(this.logger_, exception, 'Error updating Mongo', criteria);
		    	reject(error);
		    } else {
		    	resolve(doc);
		    }
...
internals.DefaultProvider.prototype.updateModel = function(resourceModel, options)
{
	var promise = promiseutils.createPromise(function(resolve, reject) {
		...
		resourceModel.save(function(error) {
		    if(error) {
		    	var exception = Exception.wrapError(error);
	    		logger.Logger.logException(this.logger_, exception, 'Error saving Mongo');
		        reject(error);
		    } else {
		        resolve(resourceModel);
		    }
...
internals.DefaultProvider.prototype.remove = function(criteria, options)
{
	...
	var promise = promiseutils.createPromise(function(resolve, reject) {
		this.Model_.remove(criteria, function(error, result) {
			...
			if(error) {
				var exception = Exception.wrapError(error);
	    		logger.Logger.logException(this.logger_, exception, 'Error removing from Mongo', criteria);
		        reject(error);
		    } else {
		        resolve(result);
		    }
