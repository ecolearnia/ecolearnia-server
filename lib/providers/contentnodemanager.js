 ...
 *
 * @fileoverview
 *  This file includes definition of ContentNodeManager class.
 *
...
internals.ContentNodeManager.prototype.count = function(criteria)
{
    return this.contentNodeProvider.count(criteria);
};
...
internals.ContentNodeManager.prototype.changeLocation = function(uuid, to, options)
{
    ...
    var promise = promiseutils.createPromise(function(resolve, reject) {
        ...
        if (!to.parentUuid) {
            var badRequest = Exception.createBadRequestError(null, {invalidProperty: 'parentUuid'});
                ...
                function processItems(seriesCallback) {
                    if (contentNode.body.items && contentNode.body.items.length > 0)
                    {
                        ...
                        function processEachItem(contentItemEntry, eachCallback) {
                            // @todo performance improvement: query by parent and do the association in-memory
