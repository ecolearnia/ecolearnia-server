 ...
 * @param {string=} prefixFilterIn  - If present, only properties that has this prefix are included 
 */
internals.HapiResource.prototype.translateCriteria = function(object, defaultCriteria, prefixFilterIn)
{
 ...
 */
internals.HapiResource.prototype.registerCustomRoute = function(server, routeConfig)
{
    var self = this;
...
internals.HapiResource.prototype.registerRoutesRecursive_ = function(server, ptr)
{
    ...
    server.route({
        ...
        handler: function(request, reply) {
            ...
            try {
                ...
                response = except.stack;
                status = 500;
                logCompletion(ptr.logger_, 'GET:failed', contextPath, status, except);
                reply(response, status);
            }
        }.bind(this)
    });
    ...
    server.route({
        ...
        handler: function(request, reply) {
            ...
            try {
                ...
                response = except.stack;
                status = 500;
                logCompletion(ptr.logger_, 'POST:failed', contextPath, status, except);
                reply(response, status);
            }
        }.bind(this)
    });
    ...
    server.route({
        ...
        handler: function(request, reply) {
            ...
            try {
                ...
                response = except.stack;
                status = 500;
                logCompletion(ptr.logger_, 'PUT:failed', contextPath, status, except);
                reply(response, status);
            }
        }.bind(this)
    });
    ...
    server.route({
        ...
        handler: function(request, reply) {
            ...
            try {
                // @todo - Validate the contentItem
                var promise = ptr.provider_.remove(criteria);
                promise.then(function(resource){
                    response = resource;
                ...
                response = except.stack;
                status = 500;
                logCompletion(ptr.logger_, 'DELETE:failed', contextPath, status, except);
                reply(response, status);
            }
        }.bind(this)
    });
